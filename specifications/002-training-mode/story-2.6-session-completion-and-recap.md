...
- Below the rank, the recap still shows the session's own statistics, under a "Détail par niveau" heading, with the same statistics layout as Story 2.3 (tiles, discreet answered/skipped line, one bar per selected level), not the profile's lifetime statistics (Story 1.5).
...
- **And** below that, it shows the session's own statistics (skipped total, found/not-found per level)
