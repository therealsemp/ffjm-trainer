...
| Story | Status | Notes |
|---|---|---|
| 3.1 — Edition selection | Done | New `/archives` route (`ArchiveSelectionPage`). `questionMetadataService.getAvailableEditions()` groups the manifest by year (most recent first), phases ordered qf → sf → fn — only combinations actually present are ever listed, one click straight into the edition's summary (3.4). |
